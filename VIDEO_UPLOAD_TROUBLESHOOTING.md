...
TOKEN=$(curl -s -X POST "https://soccer-analysis-16.preview.emergentagent.com/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"testcoach@demo.com","password":"password123"}' | \
  python3 -c "import sys, json; print(json.load(sys.stdin).get('token', ''))")

# 2. Get match ID
MATCHES=$(curl -s "https://soccer-analysis-16.preview.emergentagent.com/api/matches" \
  -H "Authorization: Bearer $TOKEN")
echo "$MATCHES"

# 3. Upload test video
MATCH_ID="<your-match-id>"
curl -X POST "https://soccer-analysis-16.preview.emergentagent.com/api/videos/upload?match_id=$MATCH_ID" \
  -H "Authorization: Bearer $TOKEN" \
...
# Test auth
curl "https://soccer-analysis-16.preview.emergentagent.com/api/auth/me" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
# List your matches
curl "https://soccer-analysis-16.preview.emergentagent.com/api/matches" \
  -H "Authorization: Bearer $TOKEN"
```
